...
class Home_controller extends Home_Core_Controller
{
    ...
    /**
     * Dynamic Promotion Page by Name Slug
     */
    public function landing_any($slug)
    {
        get_method();
        $slug = clean_slug($slug);
        //index page
        ...
        if (!empty($page)) {
            ...
            if (!empty($category)) {
                ...
                $this->product_landing_page($slug);
            }
        }
    }
     ...
     */
    public function product_landing_page($slug)
    {
        get_method();
        $slug = clean_slug($slug);
        $this->review_limit = 5;
        ...
        if (empty($data['product'])) {
            ...
            $this->load->view('partials/_header', $data);
            $this->load->view('product/details/product_landing', $data);
            $this->load->view('partials/_footer');
            //increase hit
            $this->product_model->increase_product_hit($data["product"]);
        }
