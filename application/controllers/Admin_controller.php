...
class Admin_controller extends Admin_Core_Controller
{
    ...
    */
    public function promocode()
    {
        ...
        $data['promocodes'] = $this->promocode_model->getData();

        $this->load->view('admin/includes/_header', $data);
        $this->load->view('admin/promocode/index', $data);
        $this->load->view('admin/includes/_footer');
    }

    public function validatepromocode($pc)
    {
        
        $result=$this->db->where(array('promocode'=>$pc))->get('promocode')->result_array();
    ...
    public function add_promocode()
    {
        ...
        $resultss=$this->db->where(array('promocode'=>$data['promocode']))->get('promocode')->result_array();
        if(count($resultss)==0)
        {
            $insert['promocode']=$data['promocode'];
            $insert['promocodetype']=$data['promocodetype'];
            $insert['valid_per_user']=$data['noofuse'];
            $insert['valid_for_user']=$data['foruser'];
            $insert['last_date']=date('d-m-Y',strtotime($data['enddate']));
            $insert['flat_discount']=$data['discount'];
            $insert['status']=1;//By Default active
           $result=$this->promocode_model->add_promocode($insert);
           if ($result == true) {
                $this->session->set_flashdata('success', 'msg_promocode_added');
             } else {
                $this->session->set_flashdata('error', trans("msg_error"));
            }
        }
        else{
            $this->session->set_flashdata('error', 'Please add Valid Promocode');
        }
    ...
    {
        $id = $this->input->post('id', true);
        if ($this->promocode_model->delete_promocode($id)) {
            $this->session->set_flashdata('success', trans("msg_promocode_deleted"));
        } else {
            $this->session->set_flashdata('error', trans("msg_error"));
        }
    }
