...
class Upload_model extends CI_Model
{
    ...
    public function blog_image_default_upload($path)
    {
        ...
        $img = Image::make($path);
        $img->resize(880, null, function ($constraint) {
            $constraint->aspectRatio();
        });
        $img->save(FCPATH . $new_path, $this->quality);
        //add watermark
    ...
    public function landing_page_upload($file_name)
    {   
        date_default_timezone_set('Asia/Dubai');
        $config['upload_path'] = './uploads/promotions/';
        $config['allowed_types'] = 'gif|jpg|jpeg|png|svg';
        $config['file_name'] = 'lp_' . date("Ymdhis") .'_'. uniqid();
        $this->load->library('upload', $config);

        if ($this->upload->do_upload($file_name)) {
            $data = array('upload_data' => $this->upload->data());
            if (isset($data['upload_data']['full_path'])) {
                return 'uploads/promotions/' . $data['upload_data']['file_name'];
            }
        }
        return null;
    }
