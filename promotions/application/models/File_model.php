...
class File_model extends Core_Model
{
    ...
    */
    ...
    {
        if (!$this->db->field_exists('version', 'general_settings')) {
            $this->db->query("ALTER TABLE general_settings ADD COLUMN `version` VARCHAR(30) DEFAULT '1.6.2';");
            ...
            if (!empty($languages)) {
                foreach ($languages as $language) {
                    foreach ($array as $key => $value) {
                        ...
                        $query = $this->db->get('language_translations');
                        $row = $query->row();
                        if (empty($row)) {
                            $data = array(
                                'lang_id' => $language->id,
            ...
            //delete
            $this->db->query("DELETE FROM language_translations WHERE label = 'digital_product_stock_exp';");
            $this->db->query("DELETE FROM language_translations WHERE label = 'smtp';");
            if (!$this->db->field_exists('stock_unlimited', 'products')) {
                $this->db->query("ALTER TABLE products ADD COLUMN `stock_unlimited` TINYINT(1) DEFAULT 0;");
            }
            if (!$this->db->field_exists('image_mobile', 'slider')) {
                $this->db->query("ALTER TABLE slider ADD COLUMN `image_mobile` VARCHAR(255);");
            }
            if (!$this->db->field_exists('order_product_id', 'earnings')) {
                $this->db->query("ALTER TABLE earnings ADD COLUMN `order_product_id` INT;");
            }
        }
    }
